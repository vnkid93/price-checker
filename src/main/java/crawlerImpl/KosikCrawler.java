...
public class KosikCrawler implements IPriceCheckable, ImageCrawable {
    private static final String CRAWL_URL = "https://www.kosik.cz/hledat?q=";
    public double getPrice(String EAN) throws IOException {
        double price = 0;
        Document doc = Jsoup.connect(CRAWL_URL + EAN).get();
        Elements priceElements = doc.select("p.product-item-price");
